        ...
        try:
            ...
            if self.path == '/xmpp/message':
                ...
                if self.rest._client.loggedin:
                    (cipher,sign) = m2c.utils_encrypt_sign(message)
                    ...
                    #print 'msg_send:%s \nend ...'%msg_send
                    #msg_send = {}
                    #msg_send['cipher'] = cipher
                    #msg_send['sign'] = sign
                    #for key, value in sample_dic.items():
                    #    print "\"%s\":\"%s\"" % (key, value)

                    #self.log.debug('cipher:%s '%cipher, extra={'namespace': 'xmpp'})
                    #self.log.debug('sign:%s '%sign, extra={'namespace': 'xmpp'})
                    #send_str = str(msg_send)
                    send_str = msg_send 
                    self.log.debug('send_str:%s '%send_str, extra={'namespace': 'xmpp'})
                    #self.rest._client.send_message(mto=jid, mbody=message)
                    self.rest._client.send_message(mto=jid, mbody=send_str)
                    #self.wfile.write('message [%s] to [%s] sent...' % (message, jid))
                    self.wfile.write('message [%s] to [%s] sent...' % (send_str, jid))
                else:
